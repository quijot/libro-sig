...
\begin{intro}
This chapter introduces the fundamental concepts of Geographical Information Systems (GIS). It also introduces GIS as complex systems and describes each of its main components. The chapter provides a global vision of GIS and the science of geographical information, while describing the context in which development and usage of GIS takes places nowadays.
\end{intro}

\section{Introducción}

A large part of the information that we use is georeferenced. That is, a position can be asigned to it, and is therefore information that comes along with ancilliary information about its location in space.

Information has never been as available as it is today, and we have never been in such a good context for using it and extracting the most out of it, especially if we consider the positional component that, as it has been said, most information has. This is not only because there is a large amount of georeferenced information available, but also because we are each day more aware of the importance of geographical information. Geography, which had a certain degree of relation with some other disciplines, has become now a fundamental element in most disciplines, not just in the technical or scientific contexts, but also in our daily life.

In the last decades, usage of cartography has radically changed, allowing new possibities and bringing geographic information to a much wider and diverse audience. Creation of cartography has changed itself, from being something restricted to a selected group of professionals, to being a task than now almost anyone can perform with tools that are popular and affordable. New techologies, especially collaboratives ones, allow now people to create and share cartography in a way that was not possible only a few years ago.

GIS are not just mere tools in this new context of geographical information. They are responsible for this situation, since they have themselves changed the discipline of geography into what it is now. In a society where information and technology are two of its main elements, GIS are the fundamental technology to use all kinds of geographical information, and a centralizing element for handling, analyzing and understanding all information that includes some positional component.

Therefore, GIS are basically tools to work with geographical information, a definition into which several different technologies and concepts can fit, as we will see in this book
...
Considering the example that was just described, let's try to give a more formal definition of what a GIS really is. Basically, a GIS must be able to perform the following operations:

\begin{itemize}
	\item Reading, editing, storing and, in general, managing geospatial data.
	\item Analising geospatial data. This analysis might range from simple queries to complex models, and can be referred to the spatial component of the data (the location of the element) or the thematic component (the value of the element itself).
	\item Generating results such as maps, reports, graphics, etc.
\end{itemize}

Depending of which one of these is given more prevalence, we find different definitions for a GIS. A classical defintion is the one given by \cite{Tomlin1990Prentice}, who defines a GIS a <<system is a facility for preparing, presenting, and interpreting facts that pertain to the surface of the earth. This is a broad definition\ldots a considerably narrower definition, however, is more often employed. In common parlance, a geographic information system or GIS is a configuration of computer hardware and software specifically designed for the acquisition, maintenance, and use of cartographic data.>>

\cite{Star1990Prentice} defines a GIS as a <<A geographic information system (GIS) is an information system that is designed to work with data referenced by spatial or geographic coordinates. In other words, a GIS is both a database system with specific capabilities for spatially-reference data, as well as a set of operations for working with data. In a sense, a GIS may be thought of as a higher-order map>>.

Both definitions represent the idea of a GIS at the time they were written, but the concept of a GIS nowadays needs to include other elements. The current definition of a GIS should be based on the concept of a \emph{system} as an integrating element that encompasses a set of interrelated components.

As \cite{Tomlin1990Prentice} points out, software and hardware are the two fundamental elements of a GIS. but they are not the onl ones. In the current context, other components play a role that is opf the same importance for understanding the idea of a GIS.

Also, as \cite{Star1990Prentice} states, a GIS can be considered a <<higher order map>>, that meaning that they represent a more advanced and powerful way of performing all the tasks that were traditionally performed using maps and the classical ideas and elements of cartography. That is, GIS represent a step further beyond maps. That definition, however, is too simplistic, since maps and GIS are not comparable ideas in the current context.

A map is a representation of geospatial data. Although that representation is without doubt important, in the context of a GIS it is just one element in the set of them that constitute that GIS (such the software and hardware, as it was previously mentioned). Moreover, the GIS does not contain just the geospatial data and its representation, but also all the operations and analysis that can be performed on them, which are not extraneous to the GIS itself, but a fundamental part of it as well.

Much in the same way that books have made their way from paper to computer (we used to read paper books, now we can read e--books, web pages, etc.), maps have had a similar transition since the appearance of GIS.  However, GIS is much more than a new form of cartography, and it does not invalidate previous forms. In fact, and important feature of GIS is its ability to create traditional printed maps, which are still used in most contexts. Along with this, we find other additional features that make GIS a complete and integrating tool for working with geospatial data.

Therefore, GIS must be understood as a complex system consisting of connected elements, each of them with a well--defined function. These elements, as we will detail in the following chapters, are \emph{data, analysis, visualization, technology and organizational factor}. 

With that, a more precise definition of GIS is that a GIS is system that integrates computer technology, people and geographic information \cite{webGISCOM}, its main function being to capture, analyze, store edit and represent geospatial data \cite{Korte2001Autodesk}.

In the following sections, we will see the way that GIS integrates technology, peoplea and information, an also how the fundamental underlying concepts of GIS integrate a large array of interelated disciplines.

\subsection{SIG as an integrator of information}

Different kinds of information are used in different disciplines. Finding a common element to link them and work with them is not always an easy task, although it is frequently a much needed one to be able to understand their meaning. 

Without a common ground, information used in each discipline tends to be isolated and not used in other contexts. When a 


Since most of the information has a spatial component, geography provides an ideal base ground for comparing an analizing, and acts as the linking glue between diffrent types of information that at first sight might not have much in common. 

In this context, GIS provides a framework to perform interdisciplinary projects, providing tools that can be used to work with information that is of a very diverse nature, but that has a point in common in its spatial component.
